...
public class UserController {
	...
	public ResponseEntity<APIResponse> forgotPassword(@Valid @RequestBody PasswordRequest request, @PathVariable("username") String username) {
		User user = new User();
		user.setEmail(username);
		user.setPassword(request.getPassword());
		APIResponse response = new APIResponse(service.updatePassword(user), null, null);
		return new ResponseEntity<APIResponse>(response, HttpStatus.OK);
	}
}